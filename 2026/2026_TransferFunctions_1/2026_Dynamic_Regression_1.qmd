---
title: "Transfer Function Models, additional examples"
...
head(x1_lag1)
```
We fill that missing value with zero.
...
Now we can plot the ccf values. We use `abline` to add some red dashed lines to help you identify the first lags (on both sides of zero)

```{r fig.width=12, fig.height=4}
ccf(x = x1_lag1, y = x1, lwd=2)
...
```{r fig.width=12, fig.height=4}
ccf(x = Lag(x1, shift = 1), y = x1, na.action = na.pass, lwd=2)
abline(v = -5:5, col="red", lty = 2)
```
...
**Notes about ccf:**  
 ...
 + In general **the lag k value returned by `ccf(x, y)` estimates the correlation
  between `x[t+k]` and `y[t]`**. When you apply it to the lag of x1, that is when `x[t] = x1[t - 1]` and  `y[t] = x1[t]` then the value k units to the right  shows the correlation between `x1[t - 1 + k]` and `x1[t]`. In this example,   where x1 is white noise the ony non-zero value occurs at k = 1.
 + You can use `print(ccf(...))` to see the numeric values of the ccf
 
If we use `ccf` to explore the cross correlation between the output and input of this example we get a clear indication that they are correlated for t = 0. This is a consequence of the equation model, but the (lack of) autocorrelation
...
```{r fig.width=12, fig.height=4}
ccf(x = x1, y = residuals(lm1.fit), lwd=2)
abline(v = -5:5, col="red", lty = 2)
```
...
model. Do not worry if you do not understand this function right away, we 
...
```{r fig.width=12, fig.height=4}
ccf(x = x1, y = residuals(arima1.fit), lwd=2)
abline(v = -5:5, col="red", lty = 2)
```
...
We create the output variable y2 as before.
...
TF.RegressionError.plot(y2, x2, TF.fit, lag.max = 20)
```
...
```{r fig.width=12, fig.height=4}
ccf(x = x2, y = residuals(lm2.fit), lwd=2)
abline(v = -5:5, col="red", lty = 2)
```
...
```{r fig.width=12, fig.height=4}
ccf(x = x2, y = residuals(arima2.fit), lwd=2)
abline(v = -5:5, col="red", lty = 2)
```

As you can see, this model passes all our checks.

**Exercise:** Go back to the first example and change the code line defining 
...
**Warning:** Keep in mind that in a real problem we would get the output $y_4(t)$ and the 
...
simplify the code we will get short named versions of the columns of the 
(multi)time series:
...
res[is.na(res)] <- 0
ccf(x = x2, y = res, lwd=2)
```

### Prewhitening

We said before that directly examining the ccf of input vs output is not easy 
when the input is autocorrelated. Prewhitening  is a technique that helps in 
...
```{r fig.width=12, fig.height=4}
# prewhiten(y4, x2, main="Prewhitening of y4 & x2", lwd=2)
prewhiten(x = x2, y = y4, main="Prewhitening of y4 & x2", lwd=2)
abline(v = -5:5, col="red", lty = 2)
```
...
# x2lg4 <- Lag(x2lg3, shift = 1)
# x2lg4[1] <- 0
# # fdata4[,"ar_noise"]
# # x2lg4
# y5 <- 0.7 * x2lg3 + 0.9 * x2lg4 + fdata4[,"ar_noise"]
# # y5 <- subset(y5, start = 2)
# # x2 <- subset(x2, start = 2)
# head(y5)
# head(x2)
# ccf(x = x2, y = y5, lwd=2)
# prewhiten(x = x2, y = y5)
# # abline(v = -5:5, col="red", lty = 2, lwd=1)
# abline(v = 0, col="cyan", lty = 2, lwd=2)
```
